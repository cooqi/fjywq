'use strict';
const mp_wx_data = {
	AppID: 'wx7bcde2e9584e2dc0',    
	AppSecret: '020864f49417e8c7535d060db42028e3',
}
exports.main = async (event, context) => {
    ...
    const pro_user = db.collection('user-info');    // 循环判断客户端传递过来的 action
    // 通过 action 判断请求对象
	...
	switch (event.action) {
	  ...
	  case 'code2Session':
			const res_session = await uniCloud.httpclient.request(
				...
				{
					method: 'GET', 
					data: {appid: mp_wx_data.AppID,secret: mp_wx_data.AppSecret,js_code: event.js_code,grant_type: 'authorization_code'}, 
					dataType: 'json'
				}
			)  
		...
		const success = res_session.status === 200 && res_session.data && res_session.data.openid  
			  ...
			  if(res_user.data && res_user.data.length === 0) {
				  ...
				  const register = await uniCloud.callFunction({
					name: 'user',
					data: {action: 'register',open_id: res_session.data.openid, user_info: event.user_info}
				  }).then(res => {
				   result = res
				  })
			  } else {
	  ...
	  case 'update':
		  ...
		  break;    
	  case 'getUser':
		  const res_val = await pro_user.where({  mp_wx_openid: event.open_id}).get()  
