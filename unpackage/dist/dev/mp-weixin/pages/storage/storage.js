...
const _sfc_main = {
  ...
  onLoad() {
    try {
      ...
      common_vendor.index.__f__("log", "at pages/storage/storage.vue:34", "userInfo", userInfo);
  ...
  methods: {
    getUserInfo() {
      ...
      common_vendor.index.getUserProfile({
        ...
        success: (result) => {
          ...
          common_vendor.index.__f__("log", "at pages/storage/storage.vue:60", _this.userInfo);
          _this.wxLogin();
        },
        fail: () => {
    ...
    wxLogin() {
      ...
      common_vendor.index.login({
        provider: "weixin",
        success: (res) => {
          common_vendor.index.__f__("log", "at pages/storage/storage.vue:74", "login", res);
          if (res.code) {
            common_vendor.tr.callFunction({
              ...
              success: (res2) => {
                common_vendor.index.__f__("log", "at pages/storage/storage.vue:84", "云函数返回的值：：：：", res2.result);
                common_vendor.index.hideLoading();
              ...
              fail: (err) => {
                common_vendor.index.hideLoading();
                common_vendor.index.__f__("log", "at pages/storage/storage.vue:92", "云函数调用失败", err);
              }
            });
          }
        }
    ...
    getUserTodoList(id) {
      ...
      common_vendor.tr.callFunction({
        ...
        fail: (err) => {
          common_vendor.index.hideLoading();
          common_vendor.index.stopPullDownRefresh();
          common_vendor.index.__f__("log", "at pages/storage/storage.vue:122", "云函数调用失败", err);
        }
      });
    },
    change(e) {
      common_vendor.index.__f__("log", "at pages/storage/storage.vue:127", e);
      let val = e.detail.value[0];
      common_vendor.index.showLoading({ title: "加载中", mask: true });
      let res = this.originTodoList.find((item) => item._id === val);
      common_vendor.index.__f__("log", "at pages/storage/storage.vue:132", "res", res, e);
      ...
      common_vendor.tr.callFunction({
        ...
        fail: (err) => {
          common_vendor.index.hideLoading();
          common_vendor.index.__f__("log", "at pages/storage/storage.vue:150", "云函数调用失败", err);
        }
      });
    },
    del(item) {
        ...
        success: function(res) {
          if (res.confirm) {
            ...
            common_vendor.tr.callFunction({
              ...
              fail: (err) => {
                common_vendor.index.hideLoading();
                common_vendor.index.__f__("log", "at pages/storage/storage.vue:174", "云函数调用失败", err);
              }
            });
          } else if (res.cancel) {
            common_vendor.index.__f__("log", "at pages/storage/storage.vue:178", "用户点击取消");
          }
        }
      });
    },
