  ...
  methods: {
    ...
    measureAndDraw() {
      ...
      query.select("#routeMap").boundingClientRect((rect) => {
        if (!rect) {
          common_vendor.index.__f__("error", "at pages/footprint/footprint.vue:153", "canvas 元素未找到");
          return;
        }
        common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:156", "canvas 容器尺寸:", rect.width, "x", rect.height);
    ...
    loadChinaGeo() {
      ...
      common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:171", "中国地图 GeoJSON 加载完成, 省份数:", this.chinaGeo.features.length);
    },
    // 加载足迹数据
    loadFootprint() {
      if (!this.userInfo._id)
      ...
      common_vendor._r.callFunction({
        ...
        fail: (err) => {
          this.loading = false;
          common_vendor.index.__f__("error", "at pages/footprint/footprint.vue:200", "加载足迹失败:", err);
    ...
    processFootprintData(records) {
      ...
      common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:288", "足迹数据:", this.provinceList);
      common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:289", "路线数据:", this.routeData);
      setTimeout(() => {
        this.measureAndDraw();
      }, 300);
    },
    ...
    getCityCoords(cityName) {
      ...
      common_vendor.index.__f__("warn", "at pages/footprint/footprint.vue:475", "未找到城市坐标:", cityName);
      return null;
    },
    // 计算两点间距离（Haversine公式）
    ...
    cacheStaticBackground(callback) {
      const ctx = common_vendor.index.createCanvasContext("routeMap", this);
      ...
      ctx.draw(false, () => {
        common_vendor.index.canvasToTempFilePath({
          ...
          success: (res) => {
            ...
            common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:526", "静态背景已缓存");
            callback && callback();
          },
          fail: () => {
            common_vendor.index.__f__("log", "at pages/footprint/footprint.vue:530", "缓存失败，使用直接绘制模式");
            callback && callback();
          }
        }, this);
      });
    ...
    drawChinaMap(ctx, W, H, toX, toY) {
      if (!this.chinaGeo || !this.chinaGeo.features)
        return;
      ...
      common_vendor.index.canvasToTempFilePath({
        ...
        success: (res) => {
          common_vendor.index.saveImageToPhotosAlbum({
            ...
            fail: (err) => {
              common_vendor.index.__f__("error", "at pages/footprint/footprint.vue:783", "保存失败", err);
              common_vendor.index.showToast({
                title: "保存失败",
                icon: "none"
              });
        ...
        fail: (err) => {
          common_vendor.index.__f__("error", "at pages/footprint/footprint.vue:792", "生成图片失败", err);
          common_vendor.index.showToast({
            title: "生成失败",
            icon: "none"
          });
    ...
    playDynamicRoute() {
      ...
      const duration = Math.max(2e3, totalPoints * 600);
      const interval = 100;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    b: $data.routeData.length > 0
  }, $data.routeData.length > 0 ? {
    c: $data.canvasWidth,
    d: $data.canvasHeight,
    e: common_vendor.o((...args) => $options.saveMapImage && $options.saveMapImage(...args), "90"),
    f: common_vendor.o((...args) => $options.playDynamicRoute && $options.playDynamicRoute(...args), "a3")
  } : {}, {
    g: common_vendor.t($data.provinceList.length),
    h: common_vendor.t($data.totalConcerts),
    i: common_vendor.f($data.provinceList, (province, index, i0) => {
      return {
    ...
    j: $data.provinceList.length === 0 && !$data.loading
  }, $data.provinceList.length === 0 && !$data.loading ? {} : {}), {
    k: $data.loading
  }, $data.loading ? {} : {}, {
    l: $data.showDetailPopup
  }, $data.showDetailPopup ? {
    m: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "6c")
  } : {}, {
    n: $data.showDetailPopup
  }, $data.showDetailPopup ? {
    o: common_vendor.t($data.currentProvince ? $data.currentProvince.name : ""),
    p: common_vendor.o((...args) => $options.closePopup && $options.closePopup(...args), "e9"),
    q: common_vendor.t($data.currentProvince ? $data.currentProvince.count : 0),
    r: common_vendor.f($data.currentProvince ? $data.currentProvince.records : [], (record, rIdx, i0) => {
      return {
        a: common_vendor.t(record.payType),
        b: common_vendor.t(record.city),
        c: common_vendor.t($options.formatRecordTime(record.payTime)),
        d: common_vendor.o(($event) => $options.goConcertDetail(record), rIdx),
        e: common_vendor.o(($event) => $options.goTicketCard(record), rIdx),
        f: rIdx
      };
    })
  } : {});
